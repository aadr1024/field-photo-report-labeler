...
- Process photos only from the local processing root configured by `REPORT_LABELER_LOCAL_PHOTO_ROOT`.
...
- Sync labeled-data metadata with `sync_labeled_data.py`.
- Optional metadata sync destination is configured with `REPORT_LABELER_METADATA_BACKUP_DIR`.
...
Each cadence folder always refreshes `field_photo_report_labeler_labeled_data_latest.json`. Timestamped snapshots are written only when that cadence is due, and the script does not delete old timestamped snapshots.

Photo import rule:

- Use `sync_site_photos.py` to copy missing media from the read-only Drive source into the local processing root.
...
- Recreate local photos later by rerunning `sync_site_photos.py`.

Practical threshold:

- `>25 GiB free`: import all missing photos; optimize for interactive labeling time.
